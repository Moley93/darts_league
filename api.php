...
function saveCupDraw() {
    ...
    try {
        ...
        error_log("Save cup draw raw data: " . $rawData);
        ...
        if (!isset($data['division']) || !isset($data['matches']) || !is_array($data['matches'])) {
            returnJson(['success' => false, 'error' => 'Missing required fields: division or matches']);
            return;
        }
        
        $division = $data['division'];
        $matches = $data['matches'];
        $clearExisting = isset($data['clearExisting']) ? $data['clearExisting'] : false;
        ...
        // If clearExisting is true, delete all existing cup matches for this division that aren't completed
        if ($clearExisting) {
            $stmt = $pdo->prepare("DELETE FROM matches WHERE match_type = 'cup' AND division = ? AND status != 'completed'");
            $stmt->execute([$division]);
            error_log("Cleared existing cup fixtures for $division division");
        }
        ...
        foreach ($matches as $match) {
            // Skip matches without required fields
            if (!isset($match['cup_round']) || !isset($match['division'])) {
                continue;
            }
            
            // Get team IDs (or null for TBD teams)
            $homeTeamId = null;
            $awayTeamId = null;
            
            if (!empty($match['home_team'])) {
                $stmt = $pdo->prepare("SELECT team_id FROM teams WHERE team_name = ? LIMIT 1");
                $stmt->execute([$match['home_team']]);
                $result = $stmt->fetch();
                if ($result) {
                    $homeTeamId = $result['team_id'];
                }
            }
            ...
            if (!empty($match['away_team'])) {
                ...
                $result = $stmt->fetch();
                if ($result) {
                    $awayTeamId = $result['team_id'];
                }
            }
            
            // Set default values if not provided
            $status = isset($match['status']) ? $match['status'] : 'scheduled';
            $homeScore = isset($match['home_score']) ? $match['home_score'] : 0;
            $awayScore = isset($match['away_score']) ? $match['away_score'] : 0;
            $matchDate = isset($match['match_date']) ? $match['match_date'] : date('Y-m-d');
            
            // Insert the match record
            $stmt = $pdo->prepare("
                INSERT INTO matches 
                (match_type, division, home_team_id, away_team_id, status, match_date, cup_round, home_score, away_score) 
                VALUES ('cup', ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([
                $division, 
        ...
        returnJson([
            ...
            'message' => "$matchesAdded cup matches added successfully"
        ]);
    } catch (Exception $e) {
        // Rollback on error
        ...
        error_log("Error in saveCupDraw: " . $e->getMessage());
...
/**
 * Advances cup winner to the next round
 * This function is called when a cup match is completed
 */
function advanceCupWinner($matchId) {
    ...
    try {
        ...
        // Verify this is a cup match
        if ($match['match_type'] !== 'cup') {
            error_log("Not a cup match: " . $matchId);
            return false;
        }
        
        // Check if match has a winner
        if ($match['home_score'] === $match['away_score']) {
            error_log("Match has no winner (tied): " . $matchId);
            return false;
        }
        
        // Determine winner team ID and name
        $winnerTeamId = $match['home_score'] > $match['away_score'] ? 
                         $match['home_team_id'] : $match['away_team_id'];
        $winnerTeamName = $match['home_score'] > $match['away_score'] ? 
                          $match['home_team'] : $match['away_team'];
        
        if (!$winnerTeamId) {
            error_log("Winner team ID not determined for match: " . $matchId);
            return false;
        }
        
        // Determine next round
        $currentRound = $match['cup_round'];
        $nextRound = '';
        ...
        switch ($currentRound) {
            ...
            case 'Semi Final':
                ...
                break;
            case 'Final':
                // No next round for the Final
                error_log("No next round for Final match: " . $matchId);
                return true;
            default:
                error_log("Unknown cup round: " . $currentRound);
                return false;
        }
        
        // Find the matching TBD slot in the next round
        // Logic: Find a match in the next round that has either home_team_id or away_team_id as NULL
        // and update it with the winner team ID
        
        // Lookup pattern based on round
        $lookupPattern = '';
        if ($currentRound === 'Quarter Final') {
            // For quarter finals, winners go to semi-finals based on match position
            // QF 1 & 2 winners go to SF 1, QF 3 & 4 go to SF 2
            $matchPosition = determineMatchPosition($matchId, $currentRound, $match['division']);
            if ($matchPosition <= 2) {
                $lookupPattern = "Winner of QF " . $matchPosition;
            } else {
                $lookupPattern = "Winner of QF " . $matchPosition;
            }
        } else if ($currentRound === 'Semi Final') {
            // For semi-finals, winner of SF 1 goes to Final position 1, SF 2 to position 2
            $matchPosition = determineMatchPosition($matchId, $currentRound, $match['division']);
            $lookupPattern = "Winner of SF " . $matchPosition;
        } else {
            // Generic pattern for other rounds
            $matchPosition = determineMatchPosition($matchId, $currentRound, $match['division']);
            $lookupPattern = "Winner of " . substr($currentRound, 0, 1) . " " . $matchPosition;
        }
        
        // Log the lookup
        error_log("Looking for slot with pattern: " . $lookupPattern);
        
        // Find next round match with placeholder that matches our pattern
        $stmt = $pdo->prepare("
            ...
            LEFT JOIN teams h ON m.home_team_id = h.team_id
            LEFT JOIN teams a ON m.away_team_id = a.team_id
            WHERE m.cup_round = ? AND m.division = ? AND m.match_type = 'cup'
            AND (h.team_name LIKE ? OR a.team_name LIKE ?)
        ");
        $stmt->execute([$nextRound, $match['division'], '%' . $lookupPattern . '%', '%' . $lookupPattern . '%']);
        $nextMatch = $stmt->fetch();
        
        if (!$nextMatch) {
            error_log("No matching slot found in next round " . $nextRound);
            
            // Fallback: try to find any available TBD slot
            $stmt = $pdo->prepare("
                SELECT m.match_id, m.home_team_id, m.away_team_id
                FROM matches m
                WHERE m.cup_round = ? AND m.division = ? AND m.match_type = 'cup'
                ...
                LIMIT 1
            ");
            $stmt->execute([$nextRound, $match['division']]);
            $nextMatch = $stmt->fetch();
            
            if (!$nextMatch) {
                error_log("No available slot found in next round " . $nextRound);
                return false;
            }
        }
        
        // Update the next round match with the winner team
        if ($nextMatch['home_team_id'] === null) {
            $stmt = $pdo->prepare("
                UPDATE matches SET home_team_id = ? WHERE match_id = ?
            ");
            $stmt->execute([$winnerTeamId, $nextMatch['match_id']]);
            error_log("Updated home team in match " . $nextMatch['match_id'] . " to " . $winnerTeamName);
        } else if ($nextMatch['away_team_id'] === null) {
            $stmt = $pdo->prepare("
                UPDATE matches SET away_team_id = ? WHERE match_id = ?
            ");
            $stmt->execute([$winnerTeamId, $nextMatch['match_id']]);
            error_log("Updated away team in match " . $nextMatch['match_id'] . " to " . $winnerTeamName);
        } else {
            error_log("Both slots already filled in next round match " . $nextMatch['match_id']);
            return false;
        }
        
        return true;
    } catch (Exception $e) {
        error_log("Error in advanceCupWinner: " . $e->getMessage());
        error_log("Error trace: " . $e->getTraceAsString());
        return false;
    }
}
...
/**
 * Determines the match position within its round
 */
function determineMatchPosition($matchId, $round, $division) {
    global $pdo;
    
    try {
        $stmt = $pdo->prepare("
            SELECT m1.match_id, m1.cup_round
            FROM matches m1
            WHERE m1.cup_round = ? AND m1.division = ? AND m1.match_type = 'cup'
            ORDER BY m1.match_id ASC
        ");
        $stmt->execute([$round, $division]);
        $matches = $stmt->fetchAll();
        
        // Find the position of our match in the ordered list
        foreach ($matches as $index => $match) {
            if ((int)$match['match_id'] === (int)$matchId) {
                return $index + 1;
            }
        }
        ...
        error_log("Error in determineMatchPosition: " . $e->getMessage());
        return 0;
    }
}
